...
INSERT INTO valores VALUES (4)
INSERT INTO valores VALUES (5)
INSERT INTO valores VALUES (6)
INSERT INTO valores VALUES (8)
...
SELECT TOP 1 id_anterior + 1 AS primer_espacio_libre
FROM CTE
WHERE id_anterior IS NOT NULL AND id <> id_anterior + 1
...
SELECT 
  d.producto_cod,
  SUM(CASE WHEN MONTH(f.fecha_emision) = 1 THEN d.cantidad ELSE 0 END) AS Enero,
  SUM(CASE WHEN MONTH(f.fecha_emision) = 2 THEN d.cantidad ELSE 0 END) AS Febrero,
  SUM(CASE WHEN MONTH(f.fecha_emision) = 3 THEN d.cantidad ELSE 0 END) AS Marzo,
  SUM(CASE WHEN MONTH(f.fecha_emision) = 4 THEN d.cantidad ELSE 0 END) AS Abril,
  SUM(CASE WHEN MONTH(f.fecha_emision) = 5 THEN d.cantidad ELSE 0 END) AS Mayo,
  SUM(CASE WHEN MONTH(f.fecha_emision) = 6 THEN d.cantidad ELSE 0 END) AS Junio,
  SUM(CASE WHEN MONTH(f.fecha_emision) = 7 THEN d.cantidad ELSE 0 END) AS Julio,
  SUM(CASE WHEN MONTH(f.fecha_emision) = 8 THEN d.cantidad ELSE 0 END) AS Agosto,
  SUM(CASE WHEN MONTH(f.fecha_emision) = 9 THEN d.cantidad ELSE 0 END) AS Septiembre,
  SUM(CASE WHEN MONTH(f.fecha_emision) = 10 THEN d.cantidad ELSE 0 END) AS Octubre,
  SUM(CASE WHEN MONTH(f.fecha_emision) = 11 THEN d.cantidad ELSE 0 END) AS Noviembre,
  SUM(CASE WHEN MONTH(f.fecha_emision) = 12 THEN d.cantidad ELSE 0 END) AS Diciembre
FROM facturas f
JOIN facturas_det d ON f.factura_num = d.factura_num
GROUP BY d.producto_cod
ORDER BY d.producto_cod;
...
SELECT CASE WHEN EXISTS (SELECT 1 FROM DIFERENCIAS) THEN 'son distintas' ELSE 'son iguales' END as resultado
...
SELECT c1.cliente_num, c1.cliente_ref,c2.cliente_ref,c3.cliente_ref
FROM clientes c1
...
WITH fechita as(SELECT '2023-05-10' as miFechita)
SELECT j.Nombre, j.Apellido, c.Nombre AS Club, p.FechaDesde
FROM Jugadoras j
JOIN Pases p ON j.Legajo = p.Legajo
JOIN Clubes c ON p.CodigoClub = c.Codigo
WHERE j.Legajo = 101
  AND p.FechaDesde <= (SELECT miFechita FROM fechita)
  AND NOT EXISTS (
    SELECT 1
    FROM Pases p2
    WHERE p2.Legajo = p.Legajo
      AND p2.FechaDesde <= (SELECT miFechita FROM fechita)
...
SELECT 
    ...
    CAST(SUM(fd.cantidad) * 100.0 / SUM(SUM(fd.cantidad)) OVER (PARTITION BY prov.provincia_cod) AS INT) AS [% Cant/total por Provincia],
   CAST(SUM(SUM(fd.cantidad)) OVER (PARTITION BY prov.provincia_cod) * 100.0 / SUM(SUM(fd.cantidad)) OVER () AS INT) AS [% Total Prov / Total país]
FROM facturas f
...
GROUP BY prov.provincia_cod, fd.producto_cod
ORDER BY prov.provincia_cod
