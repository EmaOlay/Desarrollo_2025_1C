...
/opt/mssql-tools/bin/sqlcmd -S localhost,1433 -U sa -P 'YourStrongPassword123!' -Q "SELECT 1" &>/dev/null
until [ $? -eq 0 ]; do
...
/opt/mssql-tools/bin/sqlcmd -S localhost,1433 -U sa -P 'YourStrongPassword123!' -Q "
    IF NOT EXISTS (SELECT name FROM master.sys.databases WHERE name = N'MyCodespaceDB')
    CREATE DATABASE MyCodespaceDB;
"

/opt/mssql-tools/bin/sqlcmd -S localhost,1433 -U sa -P 'YourStrongPassword123!' -d MyCodespaceDB -Q "
